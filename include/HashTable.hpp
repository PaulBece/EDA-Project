...
struct Timer {
    ...
    void pause() {
        if (running) {
            ...
            accumulated += std::chrono::duration<double>(now - start).count();
            running = false;
        }
    }
    ...
    double getTime() {
        if (running) {
            auto now = std::chrono::high_resolution_clock::now();
            return accumulated + std::chrono::duration<double>(now - start).count();
        }
        return accumulated;
    }
};
...
struct HashTable
{
    ...
    bool getTopWords(){
        ...
        std::string tempFilename = std::to_string(outputCounter) + ".tmp";
        
        std::string outputPath = "output/documents/";
        std::string fullFinalPath = outputPath + baseFilename;
        std::string fullTempPath = outputPath + tempFilename;
        
        std::ofstream outFile(fullTempPath);
        if (!outFile.is_open()) {
            std::cerr << "Error: Could not open file: " << fullTempPath << std::endl;
            return false;
        }
        ...
        outFile.close();
        try {
            std::filesystem::rename(fullTempPath, fullFinalPath);
